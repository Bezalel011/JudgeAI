...
class Action(Base):
    ...
    alerts = relationship("Alert", back_populates="action", cascade="all, delete-orphan")

    @property
    def evidence(self):
        """Return structured evidence dict for API responses and UI."""
        # ✅ FIXED: Safely handle various page types (int, numeric string, non-numeric string, None)
        page_value = self.evidence_page
        if page_value is None:
            page_result = None
        elif isinstance(page_value, int):
            page_result = page_value
        elif isinstance(page_value, str) and page_value.isdigit():
            page_result = int(page_value)
        else:
            # Keep as-is if not numeric (avoid crash on int() conversion)
            page_result = page_value
...
class Alert(Base):
    """Deadline alert generated by the background scheduler."""
    __tablename__ = "alerts"
    __table_args__ = (
        UniqueConstraint("action_id", "type", name="uq_alert_action_type"),
    )

    id = Column(Integer, primary_key=True, index=True)
    action_id = Column(Integer, ForeignKey("actions.id", ondelete="CASCADE"), nullable=False, index=True)
    type = Column(String, nullable=False, index=True)  # UPCOMING / OVERDUE
    message = Column(Text, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, index=True)
    is_read = Column(Boolean, default=False, nullable=False, index=True)

    action = relationship("Action", back_populates="alerts")
